...
public class CompanyController implements Initializable, IFXML {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        /*Initialize class*/
        poRecord = new XMCompany(poGRider, poGRider.getBranchCode(), false);
    ...
    private void cmdButton_Click(ActionEvent event) {
        ...
        switch (lsButton){
            case "btnBrowse":
                if (poRecord.browseRecord("%", pbLoaded)){
                    loadRecord();
    ...
    private final String pxeModuleName = "CompanyController";
    private static GRider poGRider;
    private XMCompany poRecord;
    ...
    final ChangeListener<? super Boolean> txtField_Focus = (o,ov,nv)->{
        ...
        if(!nv){ /*Lost Focus*/
            switch (lnIndex){
                ...
                case 3:
                    if (lsValue.length() > 7){
                        ShowMessageFX.Warning(getStage(), null, pxeModuleName, "Max length for 'Company Code' exceeds the maximum limit.");
                        txtField03.requestFocus();
                        return;
                    } break;
                case 4:
                    if (lsValue.length() > 15){
                        ShowMessageFX.Warning(getStage(), null, pxeModuleName, "Max length for 'Employer No.' exceeds the maximum limit.");
