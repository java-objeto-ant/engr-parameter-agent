...
import org.rmj.engr.parameter.agent.XMAffiliatedCompany;

public class AffiliatedCompanyController implements Initializable, IFXML {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        /*Initialize class*/
        poRecord = new XMAffiliatedCompany(poGRider, poGRider.getBranchCode(), false);
    ...
    private XMAffiliatedCompany poRecord;
            ...
            switch (lnIndex){
                ...
                case 3:
                    if (CommonUtils.isDate(lsValue, SQLUtil.FORMAT_SHORT_DATE)){
                        poRecord.setMaster(lnIndex, CommonUtils.toDate(lsValue));
                    } else{
                        ShowMessageFX.Warning("Invalid date entry.", pxeModuleName, "Date format must be yyyy-MM-dd (e.g. 1991-07-07)");
                        poRecord.setMaster(lnIndex, CommonUtils.toDate(java.time.LocalDate.now().toString()));
                    }
                    break;
                default:
