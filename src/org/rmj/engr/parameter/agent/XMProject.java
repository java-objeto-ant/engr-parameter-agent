...
import org.rmj.engr.parameter.base.Project;
import org.rmj.engr.parameter.pojo.UnitProject;

public class XMProject implements XMRecord{
    public XMProject(GRider foGRider, String fsBranchCD, boolean fbWithParent){
        this.poGRider = foGRider;
        if (foGRider != null){
            this.pbWithParent = fbWithParent;
            this.psBranchCd = fsBranchCD;
            
            poControl = new Project();
    ...
    public boolean browseRecord(String fsValue, boolean fbByCode){        
        JSONObject loJSON = searchProject(fsValue, fbByCode);
        
        if(loJSON == null)
            return false;
        else
            return openRecord((String) loJSON.get("sProjCode"));
    }
    
    public JSONObject searchProject(String fsValue, boolean fbByCode){
        String lsHeader = "Project Code»Description";
        String lsColName = "sProjCode»sProjDesc";
        String lsColCrit = "a.sProjCode»a.sProjDesc";
        String lsSQL = "SELECT " +
                            "  a.sProjCode" +
                            ", a.sProjDesc" + 
                            ", a.sInChrgex" + 
    ...
    private void showMessage(){
        ...
        if (!poControl.getErrMsg().isEmpty()){
            ...
            } else psErrMsgx = poControl.getErrMsg();
        } else psMessage = poControl.getMessage();
    }
    
    public String getMessage(){return psMessage;}
    public String getErrMsgx(){return psErrMsgx;}
    
    //Member Variables
    private GRider poGRider;
    private Project poControl;
    private UnitProject poData;
    ...
    private final String pxeModuleName = "XMProject";
}
